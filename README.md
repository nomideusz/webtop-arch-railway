# Deploy and Host a Linux Desktop (Arch Linux XFCE) on Railway

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/new/template/linux-desktop-arch?utm_medium=integration&utm_source=button&utm_campaign=linux-desktop-arch)

This template runs a full Arch Linux desktop with the XFCE environment in the cloud, streamed to any device through the [linuxserver.io Webtop](https://docs.linuxserver.io/images/docker-webtop/) Selkies web interface. Open your Railway domain, log in, and you have a real Linux desktop with a terminal, file manager, browser and `sudo`. Installed packages, files and settings under the home directory persist between visits.
...
**First use:** open your Railway domain, log in with `CUSTOM_USER` and the generated `PASSWORD` (service Variables tab), and the desktop appears. On touch devices, use the sidebar for keyboard and gestures. The desktop user `abc` has passwordless `sudo`, so `sudo pacman -S` works from the built-in terminal.

Notes and limits:

- Keep the login strong: the password gate is the whole security model for a desktop with sudo.
- Only `/config` (the home directory) persists. Packages installed with `pacman` live outside it and are gone after a redeploy; reinstall them or keep a setup script in your home.
- Rendering is CPU-based (no GPU on Railway). Fine for desktop apps and light video; not for 3D or heavy media work.
- Idle footprint is about 200 MB, so it boots fine on the 512 MB Trial plan. Give it 1-2 GB if you run a browser or an IDE inside.
